...
xi=[EKDspiral(1,2),EKDspiral(1,3),0,0,0,0,EKDspiral(1,6),EKDspiral(1,7)]';
...
x=zeros(1,length(EKDspiral)-1);
y=x;
theta=x;
for j=2:length(EKDspiral)
    dt=EKDspiral(j,1)-EKDspiral(j-1,1);
    m=EKDspiral(j,2:7)';
    i=EKDspiral(j,8:9)';
...
clearvars -except x y theta EKDspiral
d=0.15;
clf
...
% hold on
% plot([min(EKDspiral(:,2)), max(EKDspiral(:,2))],[EKDspiral(1,3), EKDspiral(end,3)])